...
<div className='w-[80px] h-[80px] md:w-[140px] md:h-[140px] border-2 border-black rounded-full cursor-pointer overflow-hidden'>
    ...
    <div className='w-full h-[100px] flex items-center gap-[40px] md:gap-[60px] px-[20%] pt-[30px] justify-center text-white'>
            ...
            <div className='flex items-center justify-center gap-[20px]'>
                <div className='flex relative'>
                    ...
                    {profileData?.followers?.slice(0,3).map((user, index)=>(
                     <div key={user._id || index} className={`w-[40px] h-[40px]  border-2 border-black bg-black rounded-full cursor-pointer overflow-hidden ${index>0?'-ml-[28px]':""}`}>
...
{profileData?._id==userData._id && <> {postType=="posts" &&  postData.map((post, index)=>(
        post.author?._id==profileData?._id && <Post key={post._id || index} post={post} isMyProfile={true}/>
    ))
}
